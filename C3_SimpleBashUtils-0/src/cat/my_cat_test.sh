...
my_err=my_err.txt
cat_err=cat_err.txt
files="../common/test1.txt ../common/test2.txt ../common/test3.txt
        ../common/test4.txt ../common/test6.txt
        ../common/test7.txt ../common/test8.txt ../common/test9.txt "
...
do
    ...
    let "test+=1"
    if [[ $diff != "" || `test -f $my_err` != `test -f $cat_err` ]]; then
      echo $command >> $log
      echo $diff >> $log
      echo "" >> $log
      echo "" >> $log
...
# 0 flags 2 files
echo 0 flags and 2 file >> $history
for file1 in $files
do
  for file2 in $files
  do
...
#testname="2 flags (-v and etc.) 1 file (with not printable symbols)"
echo  $testname >> $history
